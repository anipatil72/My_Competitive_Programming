...
//     {
//         rad = max(rad, a[i + 1] - a[i]);
//     }

//     // if (a.size() == 0)
//     // {

//     //     long double ans = 1.0 * l;
...
//     long double radius = (1.0) * rad / 2;

//     int tail = (int)1e9 + 7;

//     long double test = 1.0;

//     radius = max(radius, (test) * (a[0]));

//     radius = max(radius, (test) * (l - a.back()));
...
void solve()
{
    ...
    sort(v.begin(), v.end());
    cout<<fixed<<setprecision(10);
    long double ans = (long double)(v[0] - 0);
    ans = max(l - v[n - 1], ans);
    // cout <<ans <<endl ;
    for (int i = 1; i < n; i++)
    {
        long double x = (v[i] - v[i - 1]) / 2.0;
        ans = max(ans, x);
    }
    cout << ans << endl;
}
