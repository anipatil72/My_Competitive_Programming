...
int beauty(vector<vector<int>> &a, int cur, int index, int n, int m)
{
    int t = 0;
...
void solve()
{
    ...
    vector<unordered_map<int, vector<int>>> mp(m);
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        if (mp[a[i][0] - 1].count(1) > 0)
        {
            int maxi = 0;

            for (auto x : mp[a[i][0] - 1][1])
            {
                int cv = beauty(a, i, x, n, m);
                maxi = max(maxi, cv);
        ...
        // if (s[a[i][j] - 1].find() == s[a[i][j] - 1].end())
        // {
        //     ans.push_back(t);
