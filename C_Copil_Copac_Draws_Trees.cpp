...
void recur(int index, vector<vector<pair<int, int>>> &adj, int stage, int parent, int &ans, int pos)
{
    ...
    for (int i = 0; i < len; i++)
    {
        if (adj[index][i].first != parent)
        {
            int stable = stage + (pos > adj[index][i].second);

            recur(adj[index][i].first, adj, stable, index, ans, adj[index][i].second);
        }
    }
}
