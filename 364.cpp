...
const string s = "Ganapati Bappa Morya";

const int N = 1e2 + 5, MAX = 1e5;
int n, dp[N], tl;
string t;
vector<string> a;
...
int formula(int index)
{
    ...
    for (int i = 0; i < n; i++)
        if (index + a[i].length() - 1 < tl && t.substr(index, a[i].length()) == a[i])
            for (int k = 1; k <= a[i].length(); k++)
            {
                ops = min(ops, 1 + formula(index + k));
            }
