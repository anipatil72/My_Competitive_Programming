...
void solve()
{
    ...
    ll ans = (max(n, m) - 1) * (min(n, m) * maxiz) + (min(n, m) - 1) * maxia;

    ans -= (n * m - 1) * minia;

    ll fin = maxiz * (n * m - 1) - (max(m, n) - 1) * min(m, n) * minia - (min(m, n) - 1) * miniz;

    cout << max(ans, fin) << endl;
    ...
    // if (minx >= 2 || maxx >= 2)
    // {
    //     ans = (n * m - 1) * (a[n * m - 1] - a[0]);
    // }
    // else
    // {
    //     ans = max((n - 1) * (a[n * m - 1] - a[0]) + (m - 1) * (a[n * m - 2] - a[0]), (m - 1) * (a[n * m - 1] - a[0]) + (n - 1) * (a[n * m - 2] - a[0])) + (n - 1) * (m - 1) * (a[n * m - 1] - a[0]);
    // }
